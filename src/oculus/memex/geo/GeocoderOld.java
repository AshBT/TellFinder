...
public class GeocoderOld {
	...
	private static Set<String> getUngeocodablePlaces(TableDB db, Connection localConn, Set<String> places) {
		...
		for (String place : places) {
			...
			Pair<Float,Float> latlon = GeocoderOld.geocodeAlphaOnly(db, localConn, place);
			if (latlon == null || latlon.getFirst() == null || latlon.getSecond() == null) {
				result.add(place);
			}
			count++;
	...
	public static void initializeGeocoderDatabase() {
		...
		try {
			Set<String> countries = GeocoderOld.getUniqueCountries(localConn);
			Set<String> uniquePlaces = GeocoderOld.getUniquePlaces(localConn);
...
//		addValues(db, localConn);
		Pair<Float,Float> location = GeocoderOld.geocode(db, localConn, "Twin Cities, MN, USA");
